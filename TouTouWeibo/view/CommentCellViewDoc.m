...
//dave
	profileImageNode.url = [NSString stringWithFormat:@"@%@", comment.replyOfUserName];
	profileImageNode.imageUrl = comment.replyPhoto;
	authorNode.text = comment.replyOfUserName;
	
	NSMutableArray *nodes = [self parseStatus:comment.replyContext layout:commentLayout];
	[commentLayout reset];
...
- (void)setComment:(Reply *)newComment {
...
- (id)initWithComment:(Reply*)_comment width:(CGFloat)width_ {
	if (self = [self init]) {
