...
@implementation AttentionsUIViewController
...
- (void)viewDidLoad
{
    ...
    [self setViewControllerTitle:@"关注"];
    [self showWait:YES];
    currentPageNo = 1;
...
- (UITableViewCell *)tableView:(UITableView *)_tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Reply *sts = [Reply  ReplyWithAttention:[data objectAtIndex:indexPath.row] ];
...
{
    AttentionList *fanList = [CommonUtils loadAttentionList:currentPageNo andUserId:userId];
    
    NSLog( @"currentPageNo = %d userid= %@",currentPageNo,userId);
    if ( fanList.pageInfo.pageNo >= fanList.pageInfo.sumPage ) {
        [self.tableView.tableFooterView setHidden:YES];
    }else{
        [self.tableView.tableFooterView setHidden:NO];
    }
    [data addObjectsFromArray:fanList.attentionList];
