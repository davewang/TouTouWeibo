...
-(id)initWithAttention:(Attention*)_user
{
    ...
    if (self!=nil ) {
        ...
        userId = [_user.attOfUserId longLongValue];
        username = _user.attOfUserName;
        screenName = _user.attOfUserName;
        profileImageUrl = _user.headPhoto;
    }
    return self;
}
-(id)initWithWeiboModel:(WeiBoModel*)weiboModel
{
    self = [super init];
    if (self!=nil ) {
        NSLog(@"weiboModel.userId = %@",weiboModel.userId );
        
        NSLog(@"weiboModel.userName = %@",weiboModel.userName );
        userId = [weiboModel.userId longLongValue];
        username = weiboModel.userName;
        screenName = weiboModel.userName;
    }
    return self;
}
